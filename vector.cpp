...
public:
    ...
    std::vector<std::string> kafka_messages;  
    ...
    void handle_get(http_request message) {
        ...
        // Retrieve and include Kafka messages in the response
        if (!kafka_messages.empty()) {
            json::value kafka_message_array;
            for (const auto& kafka_message : kafka_messages) {
                kafka_message_array[kafka_message_array.size()] = json::value::string(U(kafka_message));
            }
            response[U("kafka_messages")] = kafka_message_array;
        }
